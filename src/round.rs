use crate::decimal::{DIGITS_PER_UNIT, FixedDecimal, UNIT, HALF_UNIT};
use crate::utils::{get_units, mod9, div9};
...
impl FixedDecimal {
    ...
    pub fn round(&mut self, frac: isize) {
        ...
        if frac >= this_frac as isize { // round precision is greater than current decimal's precision
            let round_frac_units = get_units(frac as i8);
            if round_frac_units == frac_units { // same number of frac units
                self.frac = frac as i8;
                return;
            }
            // copy with offset
            self.lsu.copy_within(..frac_units+intg_units, round_frac_units-frac_units);
            self.frac = frac as i8;
            return;
        }
        let trunc: isize;
        ...
        if frac < 0 { // round integral part
            ...
            if round_frac_units < frac_units { // this decimal has more frac units, drop them
                if mod9(frac as i8) == 0 { // at edge of one unit
                ...
                if mod9(frac as i8) == 0 { // at edge of one unit
                    if self.lsu[0] >= HALF_UNIT as i32 { // check rounding on least significant unit
                        carry = 1;
                    }
    ...
    pub fn round_to(&self, res: &mut FixedDecimal, frac: isize) {
        ...
        let intg_units = self.intg_units();
        let mut frac_units = self.frac_units();
        if frac >= this_frac as isize { // round precision is larger than or equal to current decimal's precision
            let round_frac_units = get_units(frac as i8);
            if round_frac_units == frac_units { // same number of frac units
                res.intg = self.intg;
                res.frac = frac as i8;
            ...
            // copy with offset
            res.lsu[round_frac_units - frac_units.. round_frac_units+intg_units].copy_from_slice(&self.lsu[..frac_units+intg_units]);
            res.intg = self.intg;
            res.frac = frac as i8;
            return;
        }
        ...
        if frac < 0 { // round integral part
            ...
            if round_frac_units < frac_units { // this decimal has more frac units, drop them
                ...
                res.lsu[..round_frac_units+intg_units].copy_from_slice(&self.lsu[frac_units-round_frac_units..frac_units+intg_units]);
                frac_units = round_frac_units;
            } else {
                debug_assert!(round_frac_units == frac_units);
                if mod9(frac as i8) == 0 { // at edge of one unit
                    ...
                    res.lsu[..frac_units+intg_units-1].copy_from_slice(&self.lsu[1..frac_units+intg_units]); // drop checked unit
                    frac_units -= 1;
                } else {
                    res.lsu = self.lsu;
                }
...
fn round_half_up(fd: &mut FixedDecimal, mut intg_units: usize, frac_units: usize, trunc: isize, round_frac: i8, mut carry: i32) {
    let mut round_idx = div9(trunc as i8);
    let round_pos = mod9(trunc as i8);
    let clear_idx = round_idx;
    if round_pos != 0 {
        let mut u = fd.lsu[round_idx];
        match round_pos { // unroll the code to optimize arithmetic with const values
            1 => {
                let r = u % 10;
                u -= r;
                if r >= 5 { // round up
        ...
        if u >= UNIT as i32 { // carry to higher unit
            u -= UNIT as i32;
            carry = 1;
        }
        fd.lsu[round_idx] = u;
